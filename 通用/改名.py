...
# 1.1.0
def 模型名称处理(模型:小二物体, 星穹铁道=False):
    if 模型.parent.判断类型.物体.是骨架:  # type:小二物体
        if 模型.parent.parent:
            return 模型名称(模型.parent.parent, 星穹铁道)
        return 模型名称(模型.parent, 星穹铁道)
    return 模型名称(模型, 星穹铁道)
def 模型名称(模型:小二物体, 星穹铁道):
    # 名称 = 模型.name
    名称, 后缀 = 剪去后缀(模型.name)
...
def 延迟改名(材质, 名称):
    ...
    try:
        if bpy.app.version[0] < 4:
            ...
            bpy.app.timers.register(进行, first_interval=0)
    except:
        pass
