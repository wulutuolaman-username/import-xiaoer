...
from .混合.混合贴图 import 混合贴图

def 眼口着色(self, 偏好, 数据源, 材质, 匹配贴图, alpha贴图):
    ...
    if 偏好.导入贴图:  # 如果开启了导入贴图
        ...
        if 基础贴图:
            ...
            贴图节点 = 材质.node_tree.nodes.new(type='ShaderNodeTexImage')  # 新建图像节点
            贴图节点.image = 基础贴图  # 应用贴图
            贴图节点.location = (-500, 1500)  # 定位图像节点
            材质.node_tree.links.new(
                贴图节点.outputs[0],  # 图像节点的输出插槽
