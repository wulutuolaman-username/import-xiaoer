...
# 1.1.0解决多个部件ramp问题
def ramp节点组(self, 游戏, 材质, 材质节点组, 基础贴图, 多部件, 记录次数):
###################################################################################################
    if 游戏 == "原神":
        Ramp贴图 = 获取Ramp贴图(self, 游戏, 基础贴图)
        Ramp材质节点组 = next((节点 for 节点 in 材质节点组.node_tree.nodes
                                 if 节点.type == 'GROUP' and
                                 节点.node_tree.name.startswith("ramp.")), None)
        Ramp贴图节点组 = next((节点 for 节点 in Ramp材质节点组.node_tree.nodes
                               if 节点.type == 'GROUP' and
                               节点.node_tree.name.startswith("ramptex.")), None)
        Ramp节点 = next((节点 for 节点 in Ramp贴图节点组.node_tree.nodes
                    if 节点.type == 'TEX_IMAGE'), None)  # 找到ramp贴图节点
        ...
        if Ramp贴图 and Ramp贴图.users == 0 + 记录次数:  # 1.1.0记录贴图
            ...
            Ramp节点 = next((节点 for 节点 in Ramp贴图节点组.node_tree.nodes if 节点.type == 'TEX_IMAGE'), None)  # 找到ramp贴图节点
            Ramp节点.image = Ramp贴图
...
###################################################################################################
    if 游戏 == "崩坏：星穹铁道":
        ...
        贴图节点 = next((节点 for 节点 in Ramp节点组.node_tree.nodes if 节点.type == 'TEX_IMAGE'), None)
        Ramp贴图节点 = []
        if 贴图节点:  # 找到ramp贴图节点
