...
def emotion_shader(self,prefs,data_from,diffuse_images,material):
    ...
    material.node_tree.links.new(correct_color_node_group.outputs[0], mix_node.inputs[2])
    material.node_tree.links.new(
        mix_node.outputs[0],  # 节点组的输出插槽
        output_node.inputs['Surface']  # 输出节点的输入插槽
    )
    if prefs.import_image:  # 如果开启了导入贴图
        ...
        if diffuse_image:
            ...
            material.node_tree.links.new(image_node.outputs[1], mix_node.inputs[0])
        else:
            ...
            material.node_tree.links.new(node.outputs[1], mix_node.inputs[0])