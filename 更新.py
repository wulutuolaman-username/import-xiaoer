...
# 代码来源：https://github.com/MMD-Blender/blender_mmd_tools/blob/blender-v3/mmd_tools/operators/addon_updater.py
        ...
        try:
            ...
            for r in releases:
                if _compare_version(_parse_release_version(r["tag_name"]), self.__config.min_release_version) > 0 and _compare_version(_parse_release_version(r["tag_name"]), self.__config.max_release_version) < 0:
                    ...
                    info.group = 'RELEASE'
                    info.body = r.get("body", "")  # 1.0.2新增body信息
                    self.__update_candidate.append(info)
...
class CheckAddonUpdate(bpy.types.Operator):
    bl_idname = "xiaoer.check_addon_update"
...
def register_updater(bl_info, init_py_file):
    ...
    config.owner = "wulutuolaman-username"
    config.repository = "import-xiaoer"
